package by.bsu.dstorm.localization;
...
public class LocalizationHelper {
     ...
     */
    public LocalizationHelper(int pixelsHeight, int pixelsWidth, int radiusForFitting, int threshold) {
     ...
     * @param intensities image intensity (averaged ot not)
     * @return list of maximums
     */
    ...
    private ArrayList<LocalMaximum> getThresholdAppliedMaximums(ArrayList<LocalMaximum> candidates) throws Exception {
        Collections.sort(candidates, new Comparator<LocalMaximum>() {
            ...
            public int compare(LocalMaximum o1, LocalMaximum o2) {
                return (o1.getIntensity() > o2.getIntensity()) ? -1 : ((o1.getIntensity() == o1.getIntensity()) ? 0 : 1);
            }
        });
        if (candidates.get(1).getIntensity() < threshold) {
            throw new Exception("Threshold is too big, all maximums were sieved!");
        }
        for (int i = candidates.size() - 1; i >= 0; i--) {
            if (candidates.get(i).getIntensity() < threshold) {
                candidates.remove(i);
            }
            else {
                break;
